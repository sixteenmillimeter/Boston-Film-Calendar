	...
	*/
	agx (cb) {
		...
		this.getAgx((err, d) => {
			...
			const events = d.events.filter((obj) => {
				const e = data.calAgx(obj.event)
				console.log(`${e.start_date} vs ${cutoff}`)
				if (e.start_date > cutoff) {
					return obj
				}
			})
			...
			const jobs = events.map((obj) => {
				return (next) => {
					const eventObj = data.calAgx(obj.event)
	...
	*/
	massartInsertAll (events, cb) {
		...
		const filtered = events.filter(eventObj => {
			if (eventObj.start_date > cutoff) {
				return eventObj
			}
		})

		const jobs = filtered.map(eventObj => {
			return next => {
				data.calInsert(eventObj, (err, result) => {
					if (err) {
						if (err.code == 23505) {
